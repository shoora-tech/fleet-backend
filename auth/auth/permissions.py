...
class GeoPermissions(BasePermission):
    # This will check permission for Geofence and VehicleGeofence
    def has_permission(self, request, view):
        if not request.user.is_authenticated:
            return False
        # check if the requesting user's org or branch have enabled this feature
        user = request.user
        org_id = request.user.organization_id
        roles = user.roles
        org = Organization.objects.get(uuid=org_id)
        feature = org.features.filter(name="geofence")
        if not feature:
            return False
        method = Method.objects.get(name=request.method)
        try:
